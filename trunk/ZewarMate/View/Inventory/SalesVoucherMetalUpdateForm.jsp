...
<html:form action="/Inventory/UpdateSalesMetalVoucher.do" method="post" styleId="myForm" onsubmit="updateFormObjects();">
 ...
 <div>
	<span class="FORM_TITLE_LEFT">&nbsp</span><span class="FORM_TITLE_MIDDLE" style="width:250;">Update Sales Metal Voucher</span><span class="FORM_TITLE_RIGHT" style="text-align:right;width:360;">&nbsp;</span>
 </div>
...
<!--
	...
	oGClassVoucher.ledgerEntryId = '<c:out value="${inventorySalesVoucherForm.ledgerEntryId}"/>';
	oGClassVoucher.cashVoucherId = '<c:out value="${inventorySalesVoucherForm.cashVoucherId}"/>';
	oGClassVoucher.cashVoucherEntryId = '<c:out value="${inventorySalesVoucherForm.cashVoucherEntryId}"/>';
	// Voucher Item					
	...
	function GetFormDataFromServer() {
		...
		var url="XmlSalesVoucher.do?&cache=0&itemGroupId="+oGForm.itemGroupId.value+"&voucherPrefix="+oGForm.voucherPrefix.value+"&voucherPostfix="+oGForm.voucherPostfix.value;
		...
		if (eval(oListVoucherItem)) {
			if (eval(oListVoucherItem.length) && eval(oListVoucherItem.length) > 1) {
				for (i=0; i < (oListVoucherItem.length); i++) {
					...
					oGForm.VOUCHER_ITEM_INVENTORY_PURCHASE_VOUCHER_ITEM_ID[i].name = "voucherItem["+i+"].inventorySalesVoucherItemId";
					...
					oGForm.VOUCHER_ITEM_INVENTORY_PURCHASE_VOUCHER_ITEM_ID.name = "voucherItem[0].inventorySalesVoucherItemId";
	...
	function addItemOnVoucherItem(oClassVoucherItem) {
		...
		oTr = document.createElement('tr');
			oTd = document.createElement('td');
				...
				oTd.appendChild(document.createElement('<input type="hidden" name="voucherItem[].inventorySalesVoucherItemId" id="VOUCHER_ITEM_INVENTORY_PURCHASE_VOUCHER_ITEM_ID" value="'+oClassVoucherItem.inventorySalesVoucherItemId+'" >'));
