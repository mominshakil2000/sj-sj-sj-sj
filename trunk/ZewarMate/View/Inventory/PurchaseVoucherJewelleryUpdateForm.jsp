...
<html:form action="/Inventory/UpdatePurchaseJewelleryVoucher.do" method="post" styleId="myForm" onsubmit="updateFormObjects();">
 ...
 <html:hidden property="removeInventoryPurchaseVoucherJewelleryGemIds" />
 ...
 <div>
	<span class="FORM_TITLE_LEFT">&nbsp</span><span class="FORM_TITLE_MIDDLE" style="width:250;">Update Purchase Jewellery Voucher</span><span class="FORM_TITLE_RIGHT" style="text-align:right;width:360;">&nbsp;</span>
 </div>
		...
		<table border="0" class="FORM_AREA" cellpadding="0" cellspacing="1" width="900">
			...
			<tr>
				...
				<td class="FORM_CONTROL">
					...
					<input type="hidden" name="voucherItem[0].inventoryPurchaseVoucherItemId" value="">
					<input type="hidden" name="voucherItem[0].inventoryItemEntryIdIn" value="">
		...
		<div class="FORM_AREA">
			...
			<table border="0" class="FORM_AREA" cellpadding="0" cellspacing="1" width="953">
				...
				<tr>
					...
					<td class="LIST_DATA">
						<html:hidden property="inventoryPurchaseVoucherJewelleryMetalId" /> 
						<nested:select  property="metalItemId"> 
							<html:options collection="lookupItemMetal" property="id" labelProperty="label" />
						</nested:select>
					</td>
					<td class="LIST_DATA"><html:text property="metalWeight" maxlength="6" size="6" onchange="JavaScript:calculateMetalUsedNetWeight(this);calculateMetalUsedValue(this);"/> <html:hidden property="metalWeightUnitId"/><bean:write name="inventoryPurchaseVoucherForm" property="metalWeightUnit"/></td>
					<td class="LIST_DATA">
...
<script language="javascript"><!--
	...
	function ClassVoucher() {
		...
		this.removeInventoryPurchaseVoucherJewelleryGemIds = 0;
		this.removeInventoryItemEntryIdsIn = 0;
		this.ledgerEntryId = 0;
		this.cashVoucherId = 0;
		this.cashVoucherEntryId = 0;
	...
	function ClassVoucherJewelleryMetal() {
		this.insertable = true;
		this.inventoryPurchaseVoucherJewelleryMetalId = 0;
		this.metalItemId = 0;
	...
	function ClassVoucherJewelleryGem() {
		this.insertable = true;
		this.inventoryPurchaseVoucherJewelleryGemId = 0;
		this.itemId = 0;
		this.quantity = 0;
		this.weight = 0.0;
		this.weightUnitId = 0; 
	...
	oGClassVoucher.ledgerEntryId = '<c:out value="${inventoryPurchaseVoucherForm.ledgerEntryId}"/>';
	oGClassVoucher.cashVoucherId = '<c:out value="${inventoryPurchaseVoucherForm.cashVoucherId}"/>';
	oGClassVoucher.cashVoucherEntryId = '<c:out value="${inventoryPurchaseVoucherForm.cashVoucherEntryId}"/>';	

	//Jewellery Metal
	oClassVoucherJewelleryMetal = new ClassVoucherJewelleryMetal();
	oClassVoucherJewelleryMetal.insertable = false;
	oClassVoucherJewelleryMetal.inventoryPurchaseVoucherJewelleryMetalId = '<c:out value="${inventoryPurchaseVoucherForm.inventoryPurchaseVoucherJewelleryMetalId}"/>';
	oClassVoucherJewelleryMetal.metalItemId = '<c:out value="${inventoryPurchaseVoucherForm.metalItemId}"/>';
	oClassVoucherJewelleryMetal.metalWeight = '<c:out value="${inventoryPurchaseVoucherForm.metalWeight}"/>';
	oClassVoucherJewelleryMetal.metalWeightUnitId = '<c:out value="${inventoryPurchaseVoucherForm.metalWeightUnitId}"/>'; 
	oClassVoucherJewelleryMetal.metalWeightUnit = '<c:out value="${inventoryPurchaseVoucherForm.metalWeightUnit}"/>';	
	oClassVoucherJewelleryMetal.metalWastageRate = '<c:out value="${inventoryPurchaseVoucherForm.metalWastageRate}"/>';
	oClassVoucherJewelleryMetal.metalWastageUnitId = '<c:out value="${inventoryPurchaseVoucherForm.metalWastageUnitId}"/>';
	oClassVoucherJewelleryMetal.metalNetWeight = '<c:out value="${inventoryPurchaseVoucherForm.metalNetWeight}"/>';
	oClassVoucherJewelleryMetal.metalRate = '<c:out value="${inventoryPurchaseVoucherForm.metalRate}"/>';
	oClassVoucherJewelleryMetal.metalValue = '<c:out value="${inventoryPurchaseVoucherForm.metalValue}"/>';	
	...
	<c:forEach var="item" items="${inventoryPurchaseVoucherForm.voucherJewelleryGemList}" varStatus="itemStatus" >
		...
		oGClassVoucher.oGListClassVoucherJewelleryGem[<c:out value="${itemStatus.index}"/>].inventoryPurchaseVoucherJewelleryGemId  = '<c:out value="${item.inventoryPurchaseVoucherJewelleryGemId }"/>';
	...
	function GetFormDataFromServer() {
		...
		var url="XmlPurchaseVoucher.do?&cache=0&itemGroupId="+oGForm.itemGroupId.value+"&voucherPrefix="+oGForm.voucherPrefix.value+"&voucherPostfix="+oGForm.voucherPostfix.value;
...
//			alert(oGXmlHttp.responseText);
			...
			if (vControlFlag[0]) {
				if (vControlFlag[0].getElementsByTagName("ResponseFlag")[0].text=="0")  {
					...
					oGClassVoucher.inventoryPurchaseVoucherId = oLXmlVoucher[0].getElementsByTagName("InventoryPurchaseVoucherId")[0].text;
					...
					oGClassVoucher.inventoryAccountIdCompany = oLXmlVoucher[0].getElementsByTagName("InventoryAccountIdIn")[0].text;
					...
					oGClassVoucher.cashVoucherId = oLXmlVoucher[0].getElementsByTagName("CashVoucherId")[0].text;
					oGClassVoucher.cashVoucherEntryId = oLXmlVoucher[0].getElementsByTagName("CashVoucherEntryId")[0].text;
					...
					for(var i=0; i < oLXmlVoucherItem.length; i++) {
						...
						oClassVoucherItem.inventoryPurchaseVoucherItemId  = oLXmlVoucherItem[i].getElementsByTagName("InventoryPurchaseVoucherItemId")[0].text;
						oClassVoucherItem.inventoryItemEntryIdIn = oLXmlVoucherItem[i].getElementsByTagName("InventoryItemEntryIdIn")[0].text;
					...
					for(var i=0; i < oLXmlVoucherJewelleryGem.length; i++) {
						...
						oClassVoucherJewelleryGem.inventoryPurchaseVoucherJewelleryGemId  = oLXmlVoucherJewelleryGem[i].getElementsByTagName("InventoryPurchaseVoucherJewelleryGemId")[0].text;
					...
					for(var i=0; i < oLXmlVoucherJewelleryMetal.length; i++) {
						...
						oClassVoucherJewelleryMetal.inventoryPurchaseVoucherJewelleryMetalId  = oLXmlVoucherJewelleryMetal[i].getElementsByTagName("InventoryPurchaseVoucherJewelleryMetalId")[0].text;
	...
	function loadFormData() {
		...
		oGForm.ledgerEntryId.value = oGClassVoucher.ledgerEntryId;
		oGForm.cashVoucherId.value = oGClassVoucher.cashVoucherId;
		oGForm.cashVoucherEntryId.value = oGClassVoucher.cashVoucherEntryId;		
		...
		for(var i=0; i < oGClassVoucher.oGListClassVoucherItem.length; i++) {
			...
			document.getElementsByName("voucherItem[0].inventoryPurchaseVoucherItemId")[0].value  = oGClassVoucher.oGListClassVoucherItem[i].inventoryPurchaseVoucherItemId;
			document.getElementsByName("voucherItem[0].inventoryItemEntryIdIn")[0].value = oGClassVoucher.oGListClassVoucherItem[i].inventoryItemEntryIdIn;
		...
		for(var i=0; i < oGClassVoucher.oGListClassVoucherJewelleryMetal.length; i++) {
			document.getElementsByName("inventoryPurchaseVoucherJewelleryMetalId")[0].value = oGClassVoucher.oGListClassVoucherJewelleryMetal[i].inventoryPurchaseVoucherJewelleryMetalId;
...
//			document.getElementsByName("metalWeightUnit")[0].value = oGClassVoucher.oGListClassVoucherJewelleryMetal[i].metalWeightUnit;	
	...
	function updateVoucherJewelleryGemRefrence(){
		var oListVoucherItem = GetElementsById(oGForm, "VOUCHER_JEWELLERY_GEM_INVENTORY_PURCHASE_VOUCHER_JEWELLERY_GEM_ID");
		...
		if (eval(oListVoucherItem)) {
			...
			if (eval(oListVoucherItem.length) && eval(oListVoucherItem.length) > 1) {
				...
				for (i=0; i < (oListVoucherItem.length); i++) {
					...
					oGForm.VOUCHER_JEWELLERY_GEM_INVENTORY_PURCHASE_VOUCHER_JEWELLERY_GEM_ID[i].name = "voucherJewelleryGem["+i+"].inventoryPurchaseVoucherJewelleryGemId";
					...
					oGForm.VOUCHER_JEWELLERY_GEM_INVENTORY_PURCHASE_VOUCHER_JEWELLERY_GEM_ID.name = "voucherJewelleryGem[0].inventoryPurchaseVoucherJewelleryGemId";
	...
	//	var index = oRow.rowIndex;
		...
		collection = GetElementsById(oRow.getElementsByTagName("input"), "VOUCHER_JEWELLERY_GEM_INVENTORY_PURCHASE_VOUCHER_JEWELLERY_GEM_ID");
		oGClassVoucher.removeInventoryPurchaseVoucherJewelleryGemIds+= (collection[0].value==0 ? "" : ","+collection[0].value) ;
//		alert(collection[0].value);
	...
	function saveForm(userAction) {
		...
		oGForm.removeInventoryPurchaseVoucherJewelleryGemIds.value = oGClassVoucher.removeInventoryPurchaseVoucherJewelleryGemIds;
	...
	function addItemOnVoucherJewelleryGem(oClassVoucherJewelleryGem) {
		...
		oTr = document.createElement('tr');
			oTd = document.createElement('td');
				...
				oTd.width=30;
//				oTd.appendChild(document.createElement('<input type="hidden" name="voucherJewelleryGem[].inventoryItemEntryIdIn" id="VOUCHER_ITEM_INVENTORY_ITEM_ENTRY_ID_IN" value="'+oClassVoucherJewelleryGem.inventoryItemEntryIdIn+'" >'));
//				oTd.appendChild(document.createElement('<input type="hidden" name="voucherJewelleryGem[].ledgerEntryId" id="VOUCHER_ITEM_LEDGER_ENTRY_ID" value="'+oClassVoucherJewelleryGem.ledgerEntryId+'" >'));
				oTd.appendChild(document.createElement('<input type="hidden" name="voucherJewelleryGem[].inventoryPurchaseVoucherJewelleryGemId" id="VOUCHER_JEWELLERY_GEM_INVENTORY_PURCHASE_VOUCHER_JEWELLERY_GEM_ID" value="'+oClassVoucherJewelleryGem.inventoryPurchaseVoucherJewelleryGemId+'" >'));
